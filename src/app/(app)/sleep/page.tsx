...
function TimelineBar({ segments, height = "h-5 md:h-8" }: { segments: SleepSegment[]; height?: string }) {
  return (
...
export default function SleepPage() {
  ...
  return (
    <div className="p-4 md:p-6 max-w-2xl md:max-w-3xl mx-auto">
      {/* Stats + add button */}
        ...
        <div className="flex items-center gap-2 md:gap-4 mb-2 md:mb-3">
          ...
          <div className="flex-1 relative h-3 md:h-4">
            {HOUR_TICKS.map((h) => (
              <span
                key={h}
                className="absolute -translate-x-1/2 text-[10px] md:text-xs text-[var(--treker-text-muted)] tnum"
          ...
          <div className="w-14 md:w-20 shrink-0 text-right text-[11px] md:text-xs text-[var(--treker-text-muted)]">всего</div>
        </div>

        {/* 30 rows, newest at top. Bigger gap on desktop so the taller bars
            still breathe; tight on phone where the bars are smaller. */}
          ...
          {timeline.map((row) => {
            ...
            return (
              <button
                ...
                className="w-full flex items-center gap-2 md:gap-4 rounded-md hover:bg-[var(--treker-border)]/30 active:bg-[var(--treker-border)]/50 transition-colors px-1 py-1 md:py-1.5 text-left"
              >
                <span className={cn(
                  "w-16 md:w-24 shrink-0 text-[13px] md:text-base truncate",
                ...
                <span className={cn(
                  "w-14 md:w-20 shrink-0 text-right text-[13px] md:text-base tnum",
